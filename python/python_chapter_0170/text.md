...
Почему так происходит? `super()` без аргументов эквивалентен `super(ТекущийКласс, self)` и движется по цепочке MRO. `super(Класс, self)` — это "ручное управление": поиск начинается после указанного класса, в данном случае *после* класса `B`, а это значит он сам в цепочке не участвует. Это бывает полезно для: вызова метода в обход родительского класса, решение проблем с ромбовидным наследованием или более точной настройки работы миксинов, но в тоже время и опасно - можно легко упустить особую логику работы класса.
