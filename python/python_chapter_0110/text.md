...
- `l.pop(i=-1)` — удаляет из списка `l` элемент по индексу `i` и возвращает его значение. `i` является необязательным параметром со значением -1 по умолчанию.
- `l.clear()` — удаляет из списка `l` все элементы.
- `l.sort(key=None, reverse=False)` — сортирует список `l` по возрастанию. Если `reverse` выставлен в `True`, то по убыванию. Если задан аргумент `key`, то для сравнения элементов при сортировке используется функция от одного аргумента, например `key=str.lower`.
...
В то же время операция `lst.append(x)` имеет амортизированную сложность O(1): если в выделенной области памяти хватает места, новый элемент добавляется в массив за константное время. Если же места под элемент не хватает, осуществляется аллокация новой более обширной области памяти, в которую переносятся все элементы из старой. {#block-append}
...
Операция `& ~3` выравнивает результат вниз до ближайшего числа, кратного 4. Это обеспечивает выравнивание памяти по границе 4 байта, что улучшает производительность доступа к данным на большинстве архитектур.
...
Замените значение по умолчанию для аргумента `words` с пустого списка на `None`. Проведите рефакторинг функции так, чтобы при ее вызове с аргументом `words` в значении `None`, переменная `words` в теле функции перезаписывалась пустым списком. {.task_text}
...
```
Добавьте в тело функции проверку условия: `words is None`. Если оно выполняется, присвойте переменной `words` пустой список. {.task_hint}
```python {.task_answer}
